...
class SurveyForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['title', 'description', 'header_image', 'is_active', 'starts_at', 'expires_at', 'max_responses', 'password', 'whitelist_emails', 'allow_review_response', 'send_confirmation_email', 'one_response_only']
        widgets = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Prevent selecting past dates in UI (server timezone)
        min_dt = timezone.localtime(timezone.now()).strftime('%Y-%m-%dT%H:%M')
        self.fields['starts_at'].widget.attrs['min'] = min_dt
        self.fields['expires_at'].widget.attrs['min'] = min_dt
    ...
    def clean(self):
        ...
        now = timezone.now()
        # Disallow choosing past datetime (both start and expire)
        if starts_at and starts_at < now:
            self.add_error('starts_at', 'Không được chọn thời gian bắt đầu trong quá khứ.')
        if expires_at and expires_at < now:
            self.add_error('expires_at', 'Không được chọn thời gian hết hạn trong quá khứ.')
        if starts_at and expires_at and expires_at < starts_at:
            self.add_error('expires_at', 'Thời gian hết hạn phải sau thời gian bắt đầu.')
